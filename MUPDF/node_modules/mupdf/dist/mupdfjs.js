...
export class PDFDocument extends mupdf.PDFDocument {
    ...
    getPageNumbers(label, onlyOne = false) {
        const numbers = [];
        if (!label) {
            return numbers;
        }
        ...
        if (range == undefined || range.length == 0) { // just split out all pages as single PDFs
            ...
            while (n < ranges.length) {
                ...
                if (ranges[n] != undefined) {
                    for (let o = 0; o < ranges[n].length; o++) {
    ...
    scrub(options) {
        const { attachedFiles = true, cleanPages = true, embeddedFiles = true, hiddenText = true, javascript = true, metadata = true, redactions = true, redactImages = 0, removeLinks = true, resetFields = true, resetResponses = true, thumbnails = true, xmlMetadata = true } = options;
        // Basic validation
        if (!this.isPDF()) {
...
export class PDFPage extends mupdf.PDFPage {
    ...
    insertText(value, point, fontName = "Times-Roman", fontSize = 18, graphics = { strokeColor: [0, 0, 0, 1], fillColor: [0, 0, 0, 1], strokeThickness: 1 }) {
        let doc = this._doc;
        let page = this;
        ...
        let strokeColor = graphics.strokeColor[0] + " " + graphics.strokeColor[1] + " " + graphics.strokeColor[2] + " RG";
        let fillColor = graphics.fillColor[0] + " " + graphics.fillColor[1] + " " + graphics.fillColor[2] + " rg";
        let strokeOpacity = (graphics.strokeColor[3] * 100).toString();
        let fillOpacity = (graphics.fillColor[3] * 100).toString();
        ...
        let graphicsStateIdentifier = "fitzca" + strokeOpacity + "" + fillOpacity;
        res_graphics_state.put(graphicsStateIdentifier, graphicsDict);
        let graphicsState = "/" + graphicsStateIdentifier + " gs";
        // invert the Y point
        point[1] = page.getBounds()[3] - (point[1] + fontSize);
        let contentStream = "q " + graphicsState + " BT " + strokeColor + " " + fillColor + " " + strokeThicknessMarkup + " /F1 " + fontSize + " Tf " + point[0] + " " + point[1] + " Td (" + value + ") Tj ET Q";
    ...
    insertImage(data, metrics = { x: 0, y: 0, width: 0, height: 0 }) {
        if (data.image == null) {
        ...
        let contentStream = "q " + metrics.width + " 0 0 " + metrics.height + " " + metrics.x + " " + metrics.y + " cm /" + data.name + " Do Q";
    ...
    addAnnotation(type, metrics, author, contents) {
        let page = this;
    ...
    getResourcesXrefObjects() {
        ...
        let resXObj = res.get("XObject");
        let arr = [];
        resXObj.forEach(function (value, key) {
